...
> **All `.windsurf/` files are GENERATED, never manually edited.**
...
────────────────────────────              ────────────────────────────
framework/agents/*.md            ───►     .windsurf/rules/*.md
framework/workflows/*.md         ───►     .windsurf/workflows/*.md
.vibey/config/*.yaml             ───►     .windsurfrules, settings.json
templates/windsurf/*.j2          ───►     mcp_config.json
```

### Why This Matters

1. **Prevents Drift**: Generated files always match source definitions
2. **Single Update Point**: Change `framework/workflows/feature-development.md` once, regenerate for all platforms
3. **Consistent Behavior**: Same workflow behaves identically across Claude Code, Goose, Aider, Continue, and Windsurf
4. **Version Control**: Source of truth is tracked; generated files can be `.gitignore`d

### Regeneration Commands

```bash
# Regenerate all .windsurf/ files from source
vibey deploy --platform windsurf

# Force regenerate (clears existing)
vibey deploy --platform windsurf --force

# Regenerate after framework update
vibey upgrade && vibey deploy --platform windsurf
```
...
|------|----------|-----------|
| **Configuration Drift** | High | **Never edit generated files**. All `.windsurf/` files regenerated from source on each `vibey deploy`. |
...
2. **Dynamic Regeneration (Critical)**
   - Running `vibey deploy --platform windsurf` twice produces identical output
   - Modifying `framework/workflows/feature-development.md` and regenerating updates `.windsurf/workflows/feature-development.md`
   - Generated files contain "DO NOT EDIT - Generated by Vibey" header
