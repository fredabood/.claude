...
│   │   └── [10 more workflows]
│   ├── templates/
│   │   ├── .claude/CLAUDE.md.template               # Main template
│   │   └── handoffs/
...
**A) Simple & Transparent**
   - I follow clear rules documented in .claude/CLAUDE.md
...
**D) Balanced & Discoverable [RECOMMENDED]**
   - High-level flow in .claude/CLAUDE.md + agents self-advertise when to use them
...
**Claude creates:**
```yaml
# /Users/yourname/Projects/taskmaster/.claude/project-config.yaml
...
### Step 1.6: Claude Generates .claude/CLAUDE.md
...
- Update .claude/CLAUDE.md with every change
...
- Documentation Maintenance Engineer - Keep .claude/CLAUDE.md current
...
### For Claude Code
1. Always read .claude/CLAUDE.md first
...
**Configuration:** .claude/project-config.yaml
```
...
├── src/
├── tests/
├── .claude/project-config.yaml         # ✓ Generated
...
- [ ] .claude/CLAUDE.md - Sprint 1 completion, architecture updates
...
├── .claude/project-config.yaml               # ✓ Generated by /vibey
├── .claude/CLAUDE.md                         # ✓ Generated by /vibey
├── README.md
└── .gitignore
```
...
**Claude generates `/Users/yourname/Projects/existing-api/.claude/project-config.yaml`:**
...
│   └── test_main.py
├── requirements.txt                  # ← EXISTING
├── .claude/project-config.yaml               # ✓ NEW - Generated
├── .claude/CLAUDE.md                         # ✓ NEW - Generated
...
├── src/
│   └── [application code]
├── .claude/CLAUDE.md                         # ← EXISTING .claude/CLAUDE.md (manual)
└── README.md
```
...
│   └── render-template.py
├── src/
├── .claude/CLAUDE.md                         # ← EXISTING (will be replaced/merged)
└── README.md
```

### Step 3.3: Handle Existing .claude/CLAUDE.md

**Option A: Preserve Existing Content**

```bash
# Rename existing .claude/CLAUDE.md
mv .claude/CLAUDE.md CLAUDE-original.md
```

**Option B: Merge Content (recommended)**

Keep existing .claude/CLAUDE.md for now, let Claude analyze it during `/vibey` setup
...
I also see an existing .claude/CLAUDE.md file.
...
I'm analyzing your existing .claude/CLAUDE.md to understand your project...

[Claude reads CLAUDE-original.md or .claude/CLAUDE.md]
...
*[Claude proceeds with discovery, but pre-fills answers from existing .claude/CLAUDE.md]*

**Claude:**
```
From your existing .claude/CLAUDE.md, I see:
...
**Claude generates `/Users/yourname/Projects/legacy-app/.claude/project-config.yaml`:**
...
project:
  ...
  version: "2.5.0"  # From existing .claude/CLAUDE.md
...
# PRESERVED from existing .claude/CLAUDE.md
critical_rules:
  - id: 1
...
### Step 3.6: Generate Merged .claude/CLAUDE.md

**Claude:**
```
I'm generating a new .claude/CLAUDE.md that combines:
- Your existing project context
- Your custom rules
- Vibey framework orchestration
- Updated tech stack (React migration)

Your original .claude/CLAUDE.md is saved as: CLAUDE-original.md
```
...
├── tests/                            # ← EXISTING
├── deploy.sh                         # ← EXISTING
├── .claude/project-config.yaml               # ✓ NEW - Generated with legacy rules
├── .claude/CLAUDE.md                         # ✓ NEW - Merged version
├── CLAUDE-original.md                # ✓ BACKUP - Original .claude/CLAUDE.md
└── README.md                         # ← EXISTING
```
...
### Generated Files
```
.claude/project-config.yaml                            # ⭐ Your project config
...
7. **`.claude/project-config.yaml`** - Your project configuration (generated)
8. **`.claude/CLAUDE.md`** - Your project context (generated)
