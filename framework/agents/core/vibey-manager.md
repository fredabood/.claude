...
**When You're Active:**
- User runs `/vibey` in a project with existing `project-config.yaml` and `CLAUDE.md`
...
grep -A 3 "framework:" project-config.yaml

# Show current quality gates
grep -A 10 "quality_gates:" project-config.yaml
```
...
After updating, **regenerate CLAUDE.md** to apply new mode instructions.
...
```yaml
# Update quality_gates in project-config.yaml
quality_gates:
...
**Inform User:**
"I've regenerated CLAUDE.md with your updated configuration. The new orchestration mode will take effect in your next Claude session."
...
5. Update project-config.yaml to reference custom agent
...
```yaml
# Update technology_stack in project-config.yaml
technology_stack:
  backend:
    language: "{{ language }}"
    framework: "{{ framework }}"
...
After updating, **regenerate CLAUDE.md** to reflect new tech stack.
...
# Check if CLAUDE.md is current
stat -f "%Sm" CLAUDE.md project-config.yaml
```
...
**Recommendations:**
1. Regenerate CLAUDE.md (config updated {{ days_ago }} days ago)
...
```bash
# Restore from backup
cp project-config.yaml.backup-{{ timestamp }} project-config.yaml
...
2. Regenerate CLAUDE.md with Balanced mode instructions
...
- Backup project-config.yaml before edits
- Backup CLAUDE.md before regeneration
...
1. **Never Delete User Data**
   - Never remove project-config.yaml without backup
   - Never remove CLAUDE.md without backup
...
- ✅ CLAUDE.md is regenerated and current
