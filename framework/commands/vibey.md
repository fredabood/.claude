...
# Requires both files AND the Vibey marker in .claude/CLAUDE.md
...
- Ensures .claude/CLAUDE.md was generated by Vibey
- Safe fallback: missing marker → runs initialization
...
- **Config Location:** `.claude/project-config.yaml`
...
- **Will Create:** `.claude/` directory with all framework files
...
**Triggered when:** Framework already initialized (.claude/project-config.yaml and .claude/CLAUDE.md with Vibey marker exist)
...
**Important:** All changes to .claude/project-config.yaml should be followed by regenerating .claude/CLAUDE.md
...
**Why remove .vibey?**
- Contains framework repository files (README.md, .claude/CLAUDE.md, dev docs)
...
```bash
# Check for existing .claude/project-config.yaml or .claude/CLAUDE.md
ls .claude/project-config.yaml 2>/dev/null || ls .claude/CLAUDE.md 2>/dev/null
```
...
- Pre-fill `.claude/project-config.yaml` with detected values, velocity baselines, and sprint cadence
...
- Pre-fill `.claude/project-config.yaml` with detected values (no velocity data)
...
- Pre-fill sprint cadence and velocity in `.claude/project-config.yaml`
...
3. **Generate project configuration** (`.claude/project-config.yaml`)
4. **Generate .claude/CLAUDE.md** (`.claude/CLAUDE.md` - project context for Claude)
...
- Generate Vibey files in `.claude/` directory (.claude/project-config.yaml, .claude/CLAUDE.md)
- Generate project documentation in `docs/` directory (project root)
...
**`.claude/` directory (all framework and Vibey-managed files):**
- **Framework:** agents, workflows, templates, commands, scripts, config
- **Configuration:** `.claude/project-config.yaml` - Project-specific settings
- **Context:** `.claude/CLAUDE.md` - Project context for Claude Code

**Project root (user documentation and code):**
- **Documentation:** `docs/` - Project documentation
- **Sprint Plans:** `docs/sprints/` - Sprint planning documents
- **Code:** Your project source code
