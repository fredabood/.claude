...
4. **Generate configuration**
   - Creates `project-config.yaml` with all your answers
...
Claude: ✓ Created project-config.yaml
        ✓ Generated CLAUDE.md
...
**How it works:** Claude follows keyword-based rules in CLAUDE.md
...
Claude: [Launches Documentation Engineer agent]
        [Updates README.md, API docs, CLAUDE.md]
        [Ensures consistency]
```
...
Claude: [Updates project-config.yaml]
        [Regenerates CLAUDE.md with new instructions]
        "Switched to tiered orchestration mode!"
```

Or manually edit `project-config.yaml`:
```yaml
framework:
...
5. **Customize for your needs**
   - Adjust quality gate thresholds in `project-config.yaml`
...
- `.claude/config/config-templates/ml-project-config.yaml`
