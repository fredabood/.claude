...
| Buzz Admin | buzz-relay (same process) | admin-buzz.dirtydata.studio (**tailnet-gated**, LAB-1212 — fleet block, DNS-only A → mini Tailscale IP) | buzz-relay:3000 | Read-only deployment admin dashboard (moderation reports + product feedback); enabled by `BUZZ_ADMIN_HOST`, bundle selected by the Host header (pass Host through unmodified) | Web UI (`/`, `/reports`, `/feedback`) + `GET /api/admin/v1/*`. **UNAUTHENTICATED — tailnet transport is the only control**; `curl -sI /` 404s, `/` needs `Accept: text/html`. Runbook: `submodules/memory/homelab/knowledge/buzz-runbook.md` |
