"""
Platform-specific tests for Sourcegraph Cody adapter.

Tests Cody adapter functionality including MCP configuration,
...
@pytest.fixture
def adapter():
    """Create a CodyAdapter instance with real project root."""
    return CodyAdapter(Path(__file__).parent.parent.parent)


@pytest.mark.platform
class TestCodyAdapter:
    """Test Cody adapter functionality."""

    def test_get_platform_name(self, adapter):
        """Test platform name is correct."""
        assert adapter.get_platform_name() == "cody"
    ...
    def test_get_deployment_dir(self, adapter, tmp_path):
        ...
        assert deploy_dir == tmp_path / ".cody"
    ...
    def test_supports_feature_enterprise(self, adapter):
        """Test that enterprise feature is supported."""
        assert adapter.supports_feature("enterprise") is True

    def test_supports_feature_openctx(self, adapter):
        """Test that openctx feature is supported."""
        assert adapter.supports_feature("openctx") is True
...
@pytest.mark.platform
class TestCodyContext:
    """Test Cody context file generation."""

    def test_context_contains_interfaces(self, tmp_path, mock_agent, mock_workflow):
        """Test that context lists supported interfaces."""
        with patch('vibey.adapters.cody.adapter.AgentDiscovery') as mock_ad:
            with patch('vibey.adapters.cody.adapter.WorkflowDiscovery') as mock_wd:
                mock_ad.return_value.discover.return_value = [mock_agent]
                mock_wd.return_value.discover.return_value = [mock_workflow]
                ...
                assert "Sourcegraph Web" in content

    def test_context_lists_enterprise_features(self, tmp_path, mock_agent, mock_workflow):
        """Test that context lists enterprise features."""
        with patch('vibey.adapters.cody.adapter.AgentDiscovery') as mock_ad:
            with patch('vibey.adapters.cody.adapter.WorkflowDiscovery') as mock_wd:
                mock_ad.return_value.discover.return_value = [mock_agent]
                mock_wd.return_value.discover.return_value = [mock_workflow]
    ...
    def test_context_lists_agents(self, tmp_path):
        ...
        adapter = CodyAdapter(project_root)
        output_dir = tmp_path / "output"
        adapter.export(output_dir)
    ...
    def test_context_mentions_openctx(self, tmp_path, mock_agent, mock_workflow):
        """Test that context mentions OpenCtx."""
        with patch('vibey.adapters.cody.adapter.AgentDiscovery') as mock_ad:
            with patch('vibey.adapters.cody.adapter.WorkflowDiscovery') as mock_wd:
                mock_ad.return_value.discover.return_value = [mock_agent]
                mock_wd.return_value.discover.return_value = [mock_workflow]
                ...
                assert "OpenCtx" in content


@pytest.mark.platform
class TestCodyReadme:
    """Test Cody README generation."""

    def test_readme_mentions_enterprise(self, tmp_path, mock_agent, mock_workflow):
        """Test README mentions enterprise focus."""
        with patch('vibey.adapters.cody.adapter.AgentDiscovery') as mock_ad:
            with patch('vibey.adapters.cody.adapter.WorkflowDiscovery') as mock_wd:
                mock_ad.return_value.discover.return_value = [mock_agent]
                mock_wd.return_value.discover.return_value = [mock_workflow]
                ...
                assert "July" in content  # Sunset date reference
