"""
Platform-specific tests for JetBrains AI Assistant adapter.

Tests JetBrains adapter functionality including MCP configuration,
...
@pytest.fixture
def adapter():
    """Create a JetBrainsAdapter instance with real project root."""
    return JetBrainsAdapter(Path(__file__).parent.parent.parent)


@pytest.mark.platform
class TestJetBrainsAdapter:
    """Test JetBrains adapter functionality."""
    ...
    def test_get_deployment_dir(self, adapter, tmp_path):
        ...
        assert deploy_dir == tmp_path / ".idea" / "ai"
    ...
    def test_export_mcp_config_format(self, tmp_path, mock_agent, mock_workflow):
        ...
        with patch('vibey.adapters.jetbrains.adapter.AgentDiscovery') as mock_ad:
            with patch('vibey.adapters.jetbrains.adapter.WorkflowDiscovery') as mock_wd:
                ...
                # JetBrains uses "servers" array format
                assert "servers" in config
                assert isinstance(config["servers"], list)
                assert len(config["servers"]) == 1
                assert config["servers"][0]["name"] == "vibey"
    ...
    def test_export_checksums_generated(self, adapter, tmp_path):
        ...
        assert "mcp-servers.json" in result.checksums
        assert len(result.checksums["mcp-servers.json"]) == 16
...
@pytest.mark.platform
class TestJetBrainsMCPConfig:
    """Test JetBrains MCP configuration specifics."""
...
@pytest.mark.platform
class TestJetBrainsContext:
    """Test JetBrains context file generation."""

    def test_context_contains_ide_list(self, tmp_path, mock_agent, mock_workflow):
        """Test that context lists supported IDEs."""
        with patch('vibey.adapters.jetbrains.adapter.AgentDiscovery') as mock_ad:
            with patch('vibey.adapters.jetbrains.adapter.WorkflowDiscovery') as mock_wd:
                mock_ad.return_value.discover.return_value = [mock_agent]
                mock_wd.return_value.discover.return_value = [mock_workflow]
                ...
                assert "IntelliJ IDEA" in content
                assert "PyCharm" in content
                assert "WebStorm" in content
                assert "GoLand" in content
    ...
    def test_context_lists_agents(self, tmp_path):
        ...
        adapter = JetBrainsAdapter(project_root)
        output_dir = tmp_path / "output"
        adapter.export(output_dir)
