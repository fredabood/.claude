...
VS Code has full MCP support (GA July 2025) including tools, resources,
prompts, sampling, and authentication.
...
@dataclass
class VSCodeExportResult:
    """Result of VS Code export."""
    success: bool
    output_dir: Path
...
class VSCodeAdapter(PlatformAdapter):
    """
    Adapter for VS Code native MCP support.

    Exports Vibey framework to VS Code's configuration format:
    - .vscode/mcp.json (MCP server configuration)
    - VSCODE.md context file

    VS Code has full MCP support (GA in v1.102, July 2025):
    ...
    Example:
        >>> adapter = VSCodeAdapter(Path("/path/to/vibey"))
        >>> result = adapter.export(Path("./.vscode"))
    """
    ...
    def export(
        ...
        """
        Export VS Code configuration package.

        Args:
            output_dir: Directory to write configuration (.vscode/)
            mcp_server_command: Python command for MCP server
            mcp_server_args: Args for MCP server

        Returns:
            VSCodeExportResult with metadata
        """
        if mcp_server_args is None:
            mcp_server_args = ["-m", "framework.mcp.server"]

        start_time = datetime.now(timezone.utc)
        result = VSCodeExportResult(success=False, output_dir=output_dir)
        ...
        try:
            ...
            # 1. Generate mcp.json (VS Code format)
            ...
            # 2. Generate VSCODE.md context file
            logger.info("Generating VSCODE.md...")
            ...
            result.success = True
            logger.info(f"VS Code export complete: {len(result.files_created)} files")
    ...
    def _build_mcp_config(
        ...
        """Build MCP config in VS Code format."""
        return {
            "servers": {
                "vibey": {
                    "command": command,
    ...
    def _build_context(self, agents: list, workflows: list) -> str:
        """Build VSCODE.md context content."""
        ...
        lines = [
            ...
            "## MCP Integration",
            "",
            "The Vibey MCP server is configured in `.vscode/mcp.json`.",
            "VS Code will automatically discover and connect to it.",
            "",
            "## Available Tools",
            "",
            "### Roadmap Tools",
        ...
        lines.extend([
            ...
            "<!-- Generator: vibey-vscode-adapter -->",
        ])

        return "\n".join(lines)

    def _generate_readme(self, output_dir: Path, result: VSCodeExportResult) -> Path:
        """Generate README.md."""
        content = f"""# Vibey Framework for VS Code

## Installation

The `mcp.json` file is already in the `.vscode/` directory.
VS Code will automatically detect and connect to the Vibey MCP server.

## Requirements

- VS Code 1.102 or later (MCP GA)
- Python 3.9+
...
3. Use Agent Mode or Copilot to access Vibey tools
...
"""
    ...
    def get_required_files(self) -> List[str]:
        return ["mcp.json", "VSCODE.md"]

    def supports_feature(self, feature: str) -> bool:
        return feature in {"agents", "workflows", "mcp", "roadmap", "resources", "prompts"}
