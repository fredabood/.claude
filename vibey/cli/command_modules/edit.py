...
Commands for safely editing YAML files with backup, validation, and rollback support.
"""
...
def edit_validate_cmd(file_path: Optional[str] = None, validate_all: bool = False) -> int:
    ...
    if validate_all:
        ...
        for yaml_file in yaml_files:
            ...
            if result.valid:
                valid_count += 1
                print(f"OK {yaml_file.relative_to(Path.cwd())}")
            else:
                invalid_count += 1
                print(f"FAIL {yaml_file.relative_to(Path.cwd())}")
