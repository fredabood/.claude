...
def format_status(status) -> str:
    ...
    status_map = {
        ...
        "in_progress": "B",
        "paused": "P",
        "completion_gate_check": "G",
        "completed": "C",
        "production_gate_check": "R",
        "production_ready": "!",
        "deployed": "*",
        "wont_do": "X",
    }
...
def get_object_data(fs: FileSystemManager, object_id: str) -> Dict[str, Any]:
    ...
    # Try track first
    with error_context("loading track data", raise_on_error=False):
    ...
    # Try sprint
    with error_context("loading sprint data", raise_on_error=False):
    ...
    # Try task
    with error_context("loading task data", raise_on_error=False):
...
def handle_show(args):
    ...
    if args.dir:
        ...
        with error_context("finding roadmap root"):
            root_dir = find_roadmap_root()
            if not root_dir:
                error = roadmap_not_found_error()
    ...
    # Get object data
    with error_context(f"loading object '{args.id}'"):
