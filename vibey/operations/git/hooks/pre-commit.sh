...
#
# This hook validates roadmap YAML files before allowing commits.
# It enforces YAML syntax checking and other configurable rules.
#
...
if [ -f "$REPO_ROOT/.venv/bin/python" ]; then
    ...
    echo "Error: Python not found. Cannot run pre-commit hook."
    exit 1
fi

# Run the Python pre-commit hook
exec $PYTHON -m vibey.operations.git.hooks pre-commit
