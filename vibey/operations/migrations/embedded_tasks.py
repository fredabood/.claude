...
class TaskMigrator:
    ...
    def __init__(self, root_dir: Optional[Path] = None, backup_dir: Optional[Path] = None):
        """
        Initialize migrator.
    ...
    def find_sprints_with_embedded_tasks(self) -> List[Tuple[Path, Dict]]:
        ...
        """
        sprints_with_tasks: List[Tuple[Path, Dict]] = []
    ...
    def run(self, dry_run: bool = True) -> bool:
        ...
        if dry_run:
            print(f"\n💡 This was a dry run. To execute the migration, run with execute=True")
        else:
...
def migrate_embedded_tasks(
    root_dir: Optional[Path] = None,
    dry_run: bool = True,
    backup_dir: Optional[Path] = None
) -> int:
    """
    Migrate embedded tasks to separate task files.
    ...
    Args:
        ...
        backup_dir: Directory for backups (defaults to .vibey/migration-backups/)

    Returns:
        Exit code (0 = success, non-zero = error)
    """
    migrator = TaskMigrator(root_dir=root_dir, backup_dir=backup_dir)
