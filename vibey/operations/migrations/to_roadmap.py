...
class RoadmapMigrator:
    ...
    def init_roadmap_if_needed(self, project_name: Optional[str] = None, dry_run: bool = True) -> None:
    ...
    def create_backup(self, sprint_file: Path) -> None:
...
def migrate_to_roadmap(
    ...
    """
    Migrate legacy sprint state files to roadmap system.

    Args:
        root_dir: Project root directory (defaults to current directory)
        track_id: Track ID to assign sprints to (defaults to "main")
    ...
    """
    migrator = RoadmapMigrator(root_dir=root_dir, backup=backup)
    success = migrator.run(track_id=track_id, dry_run=dry_run)
